...
public class OrderService {
    ...
    public List<OrderDto> findSortedDtos() {
        return orderRepository.findAll()
                .stream()
                .sorted(Comparator.comparing(Order::getOrderCost).reversed())
                .map(mapper::mapToDto)
                .collect(Collectors.toList());
    }
    ...
    public void createBill(Integer id) {
    ...
    private void writeBill(Bill bill) {
        try(FileWriter writer = new FileWriter("bill.txt", false))
        {
            String text = "Thank you for the purchase! \n"
                    ...
                    + "\nPayment date: " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd' 'HH:mm:ss"))
                    + "\nCost: " + bill.getCost();
